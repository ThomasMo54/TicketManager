...
export default {
  ...
  mounted() {
    ...
    axios.post("http://127.0.0.1:3000/ticket/get-tickets", {number: 50}).then(res => {
      if(res['data'] === 'Error') {
    ...
    axios.post("http://127.0.0.1:3000/user/get-users").then(res => {
  ...
  methods: {
    ...
    handleTicket: function(index) {
      axios.post("http://127.0.0.1:3000/ticket/handle-ticket", {id: this.tickets[index]._id, username: this.username}).then(res => {
        if(res['data'] === "Success") {
    ...
    unhandleTicket: function(index) {
      axios.post("http://127.0.0.1:3000/ticket/unhandle-ticket", {id: this.tickets[index]._id}).then(res => {
        if(res['data'] === "Success") {
    ...
    closeTicket: function(index) {
      axios.post("http://127.0.0.1:3000/ticket/close-ticket", {id: this.tickets[index]._id}).then(res => {
        if(res['data'] === "Success") {
    ...
    deleteTicket: function(index) {
      if(confirm('Etes-vous sûr de vouloir supprimer ce ticket ?')) {
        axios.post("http://127.0.0.1:3000/ticket/delete-ticket", {id: this.tickets[index]._id}).then(res => {
          if(res['data'] === "Success") {
    ...
    getFilteredTickets: function() {
      axios.post("http://127.0.0.1:3000/ticket/get-tickets", {number: 50, filter: this.sortBy, value: this.sortOption}).then(res => {
        if(res['data'] === 'Error') {
          this.error = 'Une erreur est survenue lors de la récuperation des tickets.';
        } else {
          this.error = '';
