  ...
  methods: {
    addUserForm: function() {
      ...
      if(this.username != '' && this.password != '' && this.confirmPassword != '') {
        if(this.password == this.confirmPassword) {
          axios.post("http://127.0.0.1:3000/user/create-user", {username: this.username, password: this.password, admin: this.adminChecked}).then(res => {
            if(res['data'] === 'Created') {
    ...
    deleteUser: function(user, index) {
      if(confirm('Etes-vous sûr de vouloir supprimer cet utilisateur ?')) {
        axios.post("http://127.0.0.1:3000/user/delete-user", {username: user}).then(res => {
          if(res['data'] === "Success") {
